...
const ActionItems = () => {
  ...
  return <section id="actie" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div ref={headerRef} className={`text-center mb-16 transition-all duration-1000 ${headerVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-8'}`}>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-2 gap-8 mb-12">
          {actions.map((action, index) => <Card key={index} className={`p-8 transition-all duration-700 border-0 bg-card ${cardsVisible ? 'opacity-100 transform translate-x-0' : 'opacity-0 transform ' + (index % 2 === 0 ? '-translate-x-12' : 'translate-x-12')}`} style={{
              ...
              <div className="flex items-start space-x-6">
                ...
                <div className="flex-1">
                  ...
                  <ul className="space-y-2 mb-6">
                    {action.features.map((feature, idx) => <li key={idx} className="flex items-center text-sm text-muted-foreground">
