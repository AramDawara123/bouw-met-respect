...
const Statistics = () => {
  ...
  useEffect(() => {
    ...
    stats.forEach((stat, index) => {
      ...
      const timer = setInterval(() => {
        ...
        setCounts(prev => ({
          ...prev,
          [index === 0 ? 'members' : index === 1 ? 'projects' : index === 2 ? 'companies' : 'success']: Math.min(Math.floor(increment * step), stat.value)
        }));
        if (step >= steps) {
          clearInterval(timer);
        }
  ...
  return <section className="py-20 bg-zinc-50">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => <Card key={index} className="p-8 text-center hover:shadow-xl transition-all duration-300 border-2 border-primary bg-card group hover:scale-105">
